...
def applyGeometricTransformation(startXs, startYs, newXs, newYs, bbox):
    ...
    for obj_idx in range(n_object):
        ...
        valid_idx = (newXs_obj != -1) & (newYs_obj != -1) & (startXs_obj != -1) & (startYs_obj != -1)
        desired_points = desired_points[valid_idx.flatten()]
        actual_points = actual_points[valid_idx.flatten()]

        if desired_points.shape[0] < 4:
            print("Skipping transformation: too few points")
            continue
        ...
        coords = np.vstack((bbox[obj_idx, :, :].T, np.ones((1, bbox[obj_idx, :, :].shape[0]))))
        new_coords = mat.dot(coords)
        newbbox[obj_idx, :, :] = new_coords[:2, :].T

        THRES = 1
        projected = mat.dot(np.vstack((desired_points.T.astype(float), np.ones((1, desired_points.shape[0])))))
        distance = np.square(projected[:2, :].T - actual_points).sum(axis=1)
